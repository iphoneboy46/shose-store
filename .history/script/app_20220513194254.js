...
function addToCartList() {
  ...
  if (productsInCart.length > 0) {
      let result =  productsInCart.map(function (product) {
      return `
            <div class="row">
            <div class="cart-img col l-3">
             <img src="${product.image}" alt="">
            ...
            <h3>Price: ${product.price} d</h3>
            <div class="quantity">
            <input type="button" class="button-minus" data-id="${product.id}" value="-">
            <span class="num">${product.number}</span>
            <input type="button" class="button-plus" data-id="${product.id}" value="+">
                 </div>
             </div>
             <div class="card-delete col l-3">
                 <i class="fa-solid fa-trash-can"></i>
...
listCard.addEventListener('click',function(e){
    ...
    if(isPlusButton || isMinusButton){
        for (let i = 0; i < productsInCart.length; i++){
            ...
            if(productsInCart[i].number <=0){
                productsInCart.slice(i, 1);
            }
        }
        addToCartList();
    }
