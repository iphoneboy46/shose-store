...
function renderProduct(albums){
    var htmlsss = albums.map(function (album){
       if(album.id <= 6){
        return `
        <div class="section__product-item product_${album.id} col l-4 m-4 c-12" data-aos="zoom-in">
...
function renderAlbumSale(albums) {
    ...
    var htmlss = albums.map(function (album){
        ...
        if(album.id == 3){
            return `
            <div class="img-sale col l-8 m-8 c-4">
            <img src="${album.image}" alt="">
        </div>
        <div class="section_img-content col l-4 m-4 c-8">
            <p>
                Sale <br>
...
function getProductInfo(product){
    ...
    fetch(cardApi,options)
        ...
        .then(function(cards){ 
            ...
            cartItem.setAttribute('data-id',`${cards.id}`);
            cartItem.innerHTML = `
            <div class="row">
                ...
                <div class="cart-details col l-6">
                    <h2 class="name-details">${cards.name}</h2>
                    <h3>Price: ${cards.price} d</h3>
                    <div class="quantity">
                        <input type="button" class="minus" onclick="Minus(${cards.number})" value="-">
                        <span class="num">${cards.number}</span>
                        <input type="button" class="plus" onclick="Plus(${cards.number})" value="+">
                    </div>
                </div>
