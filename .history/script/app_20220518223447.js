...
function getProductInfo(product){
    ...
    const cartDomItem = document.querySelectorAll('.cart__item');
    console.log({cartDomItem});
    cartDomItem.forEach(function(DomItem){
        if(DomItem.querySelector(".name-details").textContent == product.name){
            Plus(DomItem,product);
        };
    })
...
function updateShoppingCartHTML(){
    ...
    let result = productInCart.map(function(product){
            return `
            <div class="row cart__item" id="product__id">
...
function Plus(DomItem , product){
   DomItem.querySelector('.btn-plus').addEventListener('click', function(e){
       productInCart.forEach(function(productInCarts){
           if(productInCarts.id == product.id){
               DomItem.querySelector('.num').textContent = productInCarts.number;
           }
       })
   })
}
