...
function addToCartList() {
  ...
  if (productsInCart.length > 0) {
      ...
      return `
            ...
            <h3>Price: ${product.price} d</h3>
            <div class="quantity">
            <input type="button" class="minus" data-id="${product.id} value="-">
            <span class="num">${product.number}</span>
            <input type="button" class="plus" data-id="" value="+">
                 </div>
             </div>
             <div class="card-delete col l-3">
                 <i class="fa-solid fa-trash-can"></i>
...
function updateQuantityNumber(product) {
  for (let i = 0; i < productsInCart.length; i++) {
    if (productsInCart[i].id == product.id) {
      ...
      productsInCart[i].price = productsInCart[i].price * productsInCart[i].number;
      return;
    }
  }
