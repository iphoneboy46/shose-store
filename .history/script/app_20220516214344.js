...
// cart-array
var productInCart =JSON.parse(localStorage.getItem('ShoppingCart')) || []; 

function countTheSumPrice(){
    let sumPrice = 0;
    productInCart.forEach(function(product){
        sumPrice = product.price * product.number;
    });
    return sumPrice;
}
    ...
    if(productInCart.length > 0){
        let result = productInCart.map(function(product){
            ...
            <div class="cart-details col l-6">
                ...
                <div class="quantity">
                    <input type="button" class="btn-minus" onClick=Minus(${product.number}) data-id="${product.id}" value="-">
                    <span class="num">${product.number}</span>
                    <input type="button" class="btn-plus" onClick=Plus(${product.number}) data-id="${product.id}" value="+">
                </div>
            </div>
