...
function getProductInfo(product){
    ...
    fetch(cardApi,options)
        ...
        .then(function(cards){ 
            var listCartAll = document.querySelectorAll('.list-Card')
            const cartItem = document.createElement('div');
            cartItem.classList.add('card-item');
            cartItem.setAttribute('data-id',`${cards.id}`);
            for(var i = 0; i < listCartAll.length ; i++) {
                var nameT = product.querySelector('#name').textContent;
                var nameDetails = document.querySelectorAll('.name-Details').textContent;
                if(nameT == nameDetails){
